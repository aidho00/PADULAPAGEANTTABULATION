...
Public Class frmScoringPrelim
    ...
    Sub malecandidates()
        ...
        Try
            ...
            Dim sql As String
            sql = "Select (contestant_number) as 'Candidate Number', ID from tbl_contestants where contestant_gender = 'MALE' order by contestant_number asc"
            cn.Close()
            cn.Open()
            cm = New MySqlCommand(sql, cn)
            dr = cm.ExecuteReader
            While dr.Read
                dgcandidates_male.Rows.Add(dr.Item("Candidate Number").ToString, dr.Item("ID").ToString)
            End While
            dr.Close()
            cn.Close()
    ...
    Sub femalecandidates()
        ...
        Try
            ...
            Dim sql As String
            sql = "Select (contestant_number) as 'Candidate Number', ID from tbl_contestants where contestant_gender = 'FEMALE' order by contestant_number asc"
            cn.Close()
            cn.Open()
            cm = New MySqlCommand(sql, cn)
            dr = cm.ExecuteReader
            While dr.Read
                dgcandidates_female.Rows.Add(dr.Item("Candidate Number").ToString, dr.Item("ID").ToString)
            End While
            dr.Close()
            cn.Close()
        Catch ex As Exception
    ...
    Sub MalePreliminaryCriteriaList()
        Try
            prelim_criteria_male.Rows.Clear()
            Dim sql As String
            sql = "SELECT `ID`, `criteria_name`, `percentage` FROM `tbl_criterias`"
            cn.Close()
            cn.Open()
            cm = New MySqlCommand(sql, cn)
            dr = cm.ExecuteReader
            While dr.Read
                prelim_criteria_male.Rows.Add(dr.Item("ID").ToString, dr.Item("criteria_name").ToString, dr.Item("percentage").ToString)
            End While
            dr.Close()
            cn.Close()
        Catch ex As Exception
    ...
    Sub FemalePreliminaryCriteriaList()
        Try
            prelim_criteria_female.Rows.Clear()
            Dim sql As String
            sql = "SELECT `ID`, `criteria_name`, `percentage` FROM `tbl_criterias`"
            cn.Close()
            cn.Open()
            ...
            dr = cm.ExecuteReader
            While dr.Read
                prelim_criteria_female.Rows.Add(dr.Item("ID").ToString, dr.Item("criteria_name").ToString, dr.Item("percentage").ToString)
            End While
            dr.Close()
            cn.Close()
        Catch ex As Exception
    ...
    Sub MaleSubCriteriaScoring()
        Try
            ...
            Dim sql As String
            sql = "SELECT `ID`, `criteria_name`, `percentage` FROM `tbl_subcriterias` WHERE criteria_ID = " & CInt(prelim_criteria_male.CurrentRow.Cells(0).Value) & ""
            cn.Close()
            cn.Open()
            cm = New MySqlCommand(sql, cn)
            dr = cm.ExecuteReader
        ...
        Try
            For Each row As DataGridViewRow In dgcriteria_male.Rows
                Try
                    cn.Close()
                    cn.Open()
                    cm = New MySqlCommand("select ifnull(t1.judge_score, 00) from tbl_subscoring t1 where t1.subcriteria_ID = " & row.Cells(0).Value & " and t1.contestant_number = " & CInt(candidatenumber_male.Text) & " and t1.contestant_gender = 'MALE' and t1.judge_number = " & str_user_number & "", cn)
    ...
    Sub FemaleSubCriteriaScoring()
        Try
            ...
            Dim sql As String
            sql = "SELECT `ID`, `criteria_name`, `percentage` FROM `tbl_subcriterias` WHERE criteria_ID = " & CInt(prelim_criteria_female.CurrentRow.Cells(0).Value) & ""
            cn.Close()
            cn.Open()
            cm = New MySqlCommand(sql, cn)
            dr = cm.ExecuteReader
        ...
        Try
            For Each row As DataGridViewRow In dgcriteria_female.Rows
                Try
                    cn.Close()
                    cn.Open()
                    cm = New MySqlCommand("select ifnull(t1.judge_score, 00) from tbl_subscoring t1 where t1.subcriteria_ID = " & row.Cells(0).Value & " and t1.contestant_number = " & CInt(candidatenumber_female.Text) & " and t1.contestant_gender = 'FEMALE' and t1.judge_number = " & str_user_number & "", cn)
                    row.Cells(3).Value = Format(CDec(cm.ExecuteScalar), "#,##0.00")
                    cn.Close()
                Catch ex As Exception
                End Try
    ...
    Private Sub frmScoringPrelim_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        malecandidates()
        femalecandidates()
    ...
    Private Sub prelim_criteria_male_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles prelim_criteria_male.CellClick
        MaleSubCriteriaScoring()
    End Sub
    ...
    Private Sub btnSubmitMaleScore_Click(sender As Object, e As EventArgs) Handles btnSubmitMaleScore.Click
        If candidatenumber_male.Text = "#" Then
        Else
            cn.Close()
            cn.Open()
            cm = New MySqlCommand("SELECT * FROM tbl_criterias WHERE ID = " & CInt(prelim_criteria_male.CurrentRow.Cells(0).Value) & " and criteria_status = 'CLOSE'", cn)
            dr = cm.ExecuteReader
            dr.Read()
            If dr.HasRows Then
                dr.Close()
                cn.Close()
                MessageBox.Show("Criteria '" & prelim_criteria_male.CurrentRow.Cells(1).Value & "' is close.", "Submit Failed", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Else
                dr.Close()
                cn.Close()
                ...
                For Each row As DataGridViewRow In dgcriteria_male.Rows
                    cn.Close()
                    cn.Open()
                    cm = New MySqlCommand("SELECT judge_score FROM tbl_subscoring WHERE judge_number = " & str_user_number & " and contestant_number = " & CInt(candidatenumber_male.Text) & " and contestant_gender = 'MALE' and subcriteria_ID = " & CInt(row.Cells(0).Value) & "", cn)
                    dr = cm.ExecuteReader
                    dr.Read()
                    If dr.HasRows Then
                        dr.Close()
                        ...
                        query("UPDATE tbl_subscoring set judge_score  = " & subCriteriaScore & " where judge_number = " & str_user_number & " and contestant_number = " & CInt(candidatenumber_male.Text) & " and contestant_gender = 'MALE' and subcriteria_ID = " & CInt(row.Cells(0).Value) & "")
                    Else
                        dr.Close()
                        cn.Close()
                        subCriteriaPercent = CDec(row.Cells(2).Value)
                        subCriteriaScore = CDec(row.Cells(3).Value)
                        query("INSERT INTO tbl_subscoring (judge_number, contestant_ID, contestant_number, contestant_gender, subcriteria_ID, judge_score) values (" & str_user_number & ", " & CInt(dgcandidates_male.CurrentRow.Cells(1).Value) & ", " & CInt(candidatenumber_male.Text) & ", 'MALE', " & CInt(row.Cells(0).Value) & ", " & subCriteriaScore & ")")
                    End If
                Next
                ...
                cn.Close()
                cn.Open()
                cm = New MySqlCommand("SELECT judge_score FROM tbl_scoring WHERE judge_number = " & str_user_number & " and contestant_number = " & CInt(candidatenumber_male.Text) & " and contestant_gender = 'MALE' and criteria_ID = " & CInt(prelim_criteria_male.CurrentRow.Cells(0).Value) & "", cn)
                dr = cm.ExecuteReader
                dr.Read()
                If dr.HasRows Then
                    dr.Close()
                    cn.Close()
                    query("UPDATE tbl_scoring set judge_score  = " & subCriteriaScore & " where judge_number = " & str_user_number & " and contestant_number = " & CInt(candidatenumber_male.Text) & " and contestant_gender = 'MALE' and criteria_ID = " & CInt(prelim_criteria_male.CurrentRow.Cells(0).Value) & "")
                Else
                    dr.Close()
                    cn.Close()
                    query("INSERT INTO tbl_scoring (judge_number, contestant_ID, contestant_number, contestant_gender, criteria_ID, judge_score) values (" & str_user_number & ", " & CInt(dgcandidates_male.CurrentRow.Cells(1).Value) & ", " & CInt(candidatenumber_male.Text) & ", 'MALE', " & CInt(prelim_criteria_male.CurrentRow.Cells(0).Value) & ", " & subCriteriaScore & ")")
                End If

                MessageBox.Show("Male candidate number " & candidatenumber_male.Text & " - " & name_male.Text & " score for criteria '" & prelim_criteria_male.CurrentRow.Cells(1).Value & "' submitted.", "", MessageBoxButtons.OK, MessageBoxIcon.Information)
                MaleNext()
            End If
        End If
    End Sub
    ...
    Private Sub btnSubmitFemaleScore_Click(sender As Object, e As EventArgs) Handles btnSubmitFemaleScore.Click
        ...
        Else
            cn.Close()
            cn.Open()
            cm = New MySqlCommand("SELECT * FROM tbl_criterias WHERE ID = " & CInt(prelim_criteria_female.CurrentRow.Cells(0).Value) & " and criteria_status = 'CLOSE'", cn)
            dr = cm.ExecuteReader
            dr.Read()
            If dr.HasRows Then
                dr.Close()
                cn.Close()
                MessageBox.Show("Criteria '" & prelim_criteria_female.CurrentRow.Cells(1).Value & "' is close.", "Submit Failed", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Else
                dr.Close()
                cn.Close()
                Dim subCriteriaPercent As Decimal = 0
                ...
                For Each row As DataGridViewRow In dgcriteria_female.Rows
                    ...
                    cn.Close()
                    cn.Open()
                    cm = New MySqlCommand("SELECT judge_score FROM tbl_subscoring WHERE judge_number = " & str_user_number & " and contestant_number = " & CInt(candidatenumber_female.Text) & " and contestant_gender = 'FEMALE' and subcriteria_ID = " & CInt(row.Cells(0).Value) & "", cn)
                    dr = cm.ExecuteReader
                    dr.Read()
                    If dr.HasRows Then
                        dr.Close()
                        ...
                        subCriteriaPercent = CDec(row.Cells(2).Value)
                        subCriteriaScore = CDec(row.Cells(3).Value)
                        query("UPDATE tbl_subscoring set judge_score  = " & subCriteriaScore & " where judge_number = " & str_user_number & " and contestant_number = " & CInt(candidatenumber_female.Text) & " and contestant_gender = 'FEMALE' and subcriteria_ID = " & CInt(row.Cells(0).Value) & "")
                    Else
                        dr.Close()
                        cn.Close()
                        subCriteriaPercent = CDec(row.Cells(2).Value)
                        subCriteriaScore = CDec(row.Cells(3).Value)
                        query("INSERT INTO tbl_subscoring (judge_number, contestant_ID, contestant_number, contestant_gender, subcriteria_ID, judge_score) values (" & str_user_number & ", " & CInt(dgcandidates_female.CurrentRow.Cells(1).Value) & ", " & CInt(candidatenumber_female.Text) & ", 'FEMALE', " & CInt(row.Cells(0).Value) & ", " & subCriteriaScore & ")")
                ...
                cn.Close()
                cn.Open()
                cm = New MySqlCommand("SELECT judge_score FROM tbl_scoring WHERE judge_number = " & str_user_number & " and contestant_number = " & CInt(candidatenumber_female.Text) & " and contestant_gender = 'FEMALE' and criteria_ID = " & CInt(prelim_criteria_female.CurrentRow.Cells(0).Value) & "", cn)
                dr = cm.ExecuteReader
                dr.Read()
                If dr.HasRows Then
                    dr.Close()
                    cn.Close()
                    query("UPDATE tbl_scoring set judge_score  = " & subCriteriaScore & " where judge_number = " & str_user_number & " and contestant_number = " & CInt(candidatenumber_female.Text) & " and contestant_gender = 'FEMALE' and criteria_ID = " & CInt(prelim_criteria_female.CurrentRow.Cells(0).Value) & "")
                Else
                    dr.Close()
                    cn.Close()
                    query("INSERT INTO tbl_scoring (judge_number, contestant_ID, contestant_number, contestant_gender, criteria_ID, judge_score) values (" & str_user_number & ", " & CInt(dgcandidates_female.CurrentRow.Cells(1).Value) & ", " & CInt(candidatenumber_female.Text) & ", 'FEMALE', " & CInt(prelim_criteria_female.CurrentRow.Cells(0).Value) & ", " & subCriteriaScore & ")")
                End If

                MessageBox.Show("Female candidate number " & candidatenumber_female.Text & " - " & name_female.Text & " score for criteria '" & prelim_criteria_female.CurrentRow.Cells(1).Value & "' submitted.", "", MessageBoxButtons.OK, MessageBoxIcon.Information)
                FemaleNext()
            End If
        End If
    End Sub
    ...
    Private Sub prelim_criteria_female_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles prelim_criteria_female.CellContentClick

    End Sub

    Private Sub prelim_criteria_male_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles prelim_criteria_male.CellContentClick

    End Sub

    Private Sub prelim_criteria_female_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles prelim_criteria_female.CellClick
        FemaleSubCriteriaScoring()
    End Sub

    Private Sub PictureBox_male_Click(sender As Object, e As EventArgs) Handles PictureBox_male.Click
        frmPhoto.PictureBox1.Image = PictureBox_male.Image
        OpenForm(frmPhoto)
    End Sub

    Private Sub PictureBox_female_Click(sender As Object, e As EventArgs) Handles PictureBox_female.Click
        frmPhoto.PictureBox1.Image = PictureBox_female.Image
        OpenForm(frmPhoto)
    End Sub
End Class
